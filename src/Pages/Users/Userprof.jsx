            ...
            .map((b) => (
              <div className={"card you"} key={`${b.lot}-${b.slot}`}>
                ...
                <div className="card-right">
                  ...
                  {services && services.filter((s) => s.lotIndex === b.lotIndex && s.slot === b.slot).length > 0 && (
                    <div className="services-for-booking">
                      {services.filter((s) => s.lotIndex === b.lotIndex && s.slot === b.slot).map((svc) => (
                        <div className="service-pill" key={svc.id}>{svc.serviceTitle} — {new Date(svc.bookedAt).toLocaleString()}</div>
                      ))}
                    </div>
                  )}
                </div>
